...
import { bfs, generateRandomGraph, Graph, Node, Edge } from '@/lib/algorithms/traversal';

export default function BFSPage() {
  const [graph, setGraph] = useState(generateRandomGraph(5, 0.3));
  const [steps, setSteps] = useState<ReturnType<typeof bfs>>([]);
  ...
  const generateGraph = () => {
    if (showCustomGraph) {
      ...
      const newSteps = bfs(newGraph, validStartNode);
      setSteps(newSteps);
      setIsPlaying(false);
      setIsComplete(false);
  ...
  useEffect(() => {
    if (graph && graph.nodes.length > 0) {
      const newSteps = bfs(graph, startNode);
      setSteps(newSteps);
      setIsPlaying(false);
      setIsComplete(false);
      setIsInitialized(true);
  ...
  // Handle adjacency list change
  const handleListChange = (nodeIndex: number, value: string) => {
    const node = nodeLabels[nodeIndex];
    
    // Split the input by commas and trim spaces
    const connections = value.split(',')
      .map(s => s.trim())
      .filter(s => s !== '');
    
    const newList = {...adjacencyList};
    newList[node] = connections;
    setAdjacencyList(newList);
  };
  ...
  const createCustomGraph = () => {
    ...
    const newSteps = bfs(customGraph, validStartNode);
    setSteps(newSteps);
    setIsPlaying(false);
    setIsComplete(false);
    setIsInitialized(true);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <h1 className="text-4xl font-bold mb-6">Breadth First Search</h1>
  ...
  return { visited: Array.from(visited), distances };
}`}</code>
        </pre>
      </div>
    </div>
